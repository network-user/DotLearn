...
export const LibraryPage = () => {
  ...
  return (
    <div className="space-y-8">
      ...
      {tab === 'highlights' && counts.highlights > 0 ? <HighlightLegend /> : null}
  ...
  return (
    ...
    >
      {HIGHLIGHT_COLORS.map((color) => (
        <span key={color.id} className="inline-flex items-center gap-1.5 text-[12px] text-fg-muted">
          <span aria-hidden className={cx('size-2.5 rounded-full', color.swatchClass)} />
          {t(`highlight.colors.${color.id}`)}
        </span>
      ))}
    </div>
  );
...
const HighlightCard = ({ item, titles, onMakeCard }: HighlightCardProps) => {
  ...
  const saveNote = (): void => {
    void setHighlightNote(item.id, noteValue);
    setEditing(false);
  };
      ...
      <Surface variant="chrome" className="p-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0 flex-1">
            <div className="flex items-start gap-2">
              <span className="mt-0.5 inline-flex shrink-0 items-center gap-1.5">
                ...
                <span
                  aria-hidden
                  className={cx('size-2.5 rounded-full', highlightColorOf(item.color).swatchClass)}
                />
              </span>
              <blockquote
                className={cx(
                  'border-l-2 pl-3 font-serif text-[15px] text-fg',
                  highlightColorOf(item.color).borderClass,
                )}
              >
                {item.text}
              </blockquote>
            ...
            {editing ? (
              <div className="mt-3 space-y-2.5">
                <HighlightColorPicker
                  size="sm"
                  value={item.color}
                  onSelect={(color) => void setHighlightColor(item.id, color)}
                />
                <textarea
                  value={noteValue}
                  onChange={(event) => setNoteValue(event.target.value)}
                  placeholder={t('highlightNote.placeholder')}
                  aria-label={t('highlightNote.label')}
                  rows={2}
              ...
              <p className="mt-1 text-[13px] text-fg-muted">{item.note}</p>
            ) : null}
            <div className="mt-2">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <IconButton
              label={item.note ? t('highlightNote.edit') : t('highlightNote.add')}
