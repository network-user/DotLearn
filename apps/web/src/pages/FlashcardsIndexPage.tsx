...
interface InterviewDirectionGroup {
  direction: DirectionConfig['id'];
  label: string;
  total: number;
...
export const FlashcardsIndexPage = () => {
  ...
  const [interviewGroups, setInterviewGroups] = useState<InterviewDirectionGroup[] | undefined>(
    undefined,
  );
  ...
  useEffect(() => {
    ...
    void loadInterviewCards(language).then((cards) => {
      if (cancelled) return;
      const byDirection = new Map<DirectionConfig['id'], InterviewDirectionGroup>();
      for (const entry of getInterviewDirections()) {
        byDirection.set(entry.id, {
          direction: entry.id,
          label: directionLabel(entry.id, language),
          total: 0,
          categories: [],
        });
      }
      ...
      for (const card of cards) {
        const directionId = card.direction;
        if (!directionId) continue;
        const group = byDirection.get(directionId);
      ...
      for (const summary of byCategory.values()) {
        const sample = cards.find((card) => card.category === summary.category);
        const directionId = sample?.direction;
        if (!directionId) continue;
        const group = byDirection.get(directionId);
        if (group) group.categories.push(summary);
      }
      const rows = [...byDirection.values()]
        .filter((group) => group.total > 0)
        .map((group) => ({
          ...group,
          categories: group.categories.sort((a, b) => a.label.localeCompare(b.label, 'ru')),
        }));
      setInterviewGroups(rows);
    });
    return () => {
      cancelled = true;
    };
  ...
  const ready = summaries !== undefined && interviewGroups !== undefined;

  const reviewAllInterviewSearch = (direction?: DirectionConfig['id']) => ({
  ...
  const reviewCategorySearch = (category: string, direction?: DirectionConfig['id']) => ({
    mode: 'interview' as const,
    category,
    ...(direction ? { direction } : {}),
    due: 'all' as const,
            ...
            {interviewGroups!.map((group) => (
              <div key={group.direction} className="space-y-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <Link
                    to="/flashcards/practice"
                    search={reviewAllInterviewSearch(group.direction)}
                  >
                    <Button variant="outline" size="sm" className="w-full sm:w-auto">
                      {t('reviewDirection', { direction: group.label })}
                    </Button>
                  </Link>
                </div>
                <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
                  {group.categories.map((summary) => (
                    <li key={summary.category}>
                      <Surface interactive className="h-full">
                        <div className="flex h-full flex-col gap-4 p-5">
                          ...
                          >
                            <div className="flex items-start justify-between gap-3">
                              <h4 className="min-w-0 font-display text-xl leading-tight tracking-tightish text-fg">
                                {summary.label}
                              </h4>
                          ...
                          <div className="mt-auto flex flex-col gap-2 sm:flex-row">
                            ...
                            <Link
                              to="/flashcards/practice"
                              search={reviewCategorySearch(summary.category, group.direction)}
                              className="flex-1"
                            >
