...
import { loadTopicCards, type DeckCard } from '@/lib/flashcard-decks';
...
type Phase = 'loading' | 'nodeck' | 'caught-up' | 'review' | 'done';
...
export const FlashcardReviewPage = () => {
  ...
  const buildQueue = useCallback(
    async (loaded: DeckCard[]): Promise<number[]> => {
      const records = await db.flashcardReviews.where('topicSlug').equals(slug).toArray();
      const byCard = new Map(records.map((record) => [record.cardId, record]));
      const now = Date.now();
      return loaded
        .map((_, index) => index)
        .filter((index) => {
          const record = byCard.get(loaded[index]!.id);
  ...
  useEffect(() => {
    ...
    loadTopicCards(slug, language)
      .then(async (loaded) => {
        ...
        const due = await buildQueue(loaded);
        if (cancelled) return;
        setQueue(due);
        setPhase(due.length === 0 ? 'caught-up' : 'review');
      })
      .catch(() => {
        if (!cancelled) setPhase('nodeck');
      });
    return () => {
      cancelled = true;
    };
  }, [slug, language, buildQueue]);
  ...
  const rate = useCallback(
    (rating: FlashcardRating) => {
      if (!current) return;
      void reviewFlashcard(slug, current.id, rating);
      setReviewed((value) => value + 1);
      advance();
    },
    [current, slug, advance],
  );
      ...
      {phase === 'nodeck' && (
        <Surface variant="inset">
          <div className="p-8 text-center">
            <p className="text-sm text-fg-muted">{t('noDeck')}</p>
        ...
        <Surface variant="inset" rule="top">
          <div className="p-8 text-center space-y-4">
            ...
            <p className="text-sm text-fg-muted">
              {t('caughtUpBody', { count: cards.length })}
            </p>
