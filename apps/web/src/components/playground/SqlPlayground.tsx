...
import { Database, Eraser, Play, RotateCcw } from 'lucide-react';
...
import { saveSqlState, type SqlPlaygroundState } from '@/lib/playground';
import {
  defaultSqlTemplate,
  sqlTemplateById,
  sqlTemplates,
...
export const SqlPlayground = ({ initialState }: SqlPlaygroundProps) => {
  ...
  useEffect(() => {
    void saveSqlState({ templateId, schema, query });
  }, [templateId, schema, query]);
  ...
  const handleSelectTemplate = useCallback(
    (id: string): void => {
      const template = sqlTemplateById(id) ?? defaultSqlTemplate;
  ...
  const handleReset = useCallback((): void => {
    const template = sqlTemplateById(templateId) ?? defaultSqlTemplate;
    applyTemplate(template);
