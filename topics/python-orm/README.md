...
How Python talks to a database: from low-level DB-API (`sqlite3`) with parameterization and transactions to SQLAlchemy 2.x ORM in modern style. This means declarative models with `Mapped`, queries via `select()`, `relationship()` links, and fixing the N+1 problem.
...
Python runs entirely in your browser via `pyodide`. No setup.
