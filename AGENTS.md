...
> Перегенерировано скиллом `sync-project-rules` / `generate-readme`. Источник правды - код репозитория, не этот файл.
...
├── topics/         # 62 темы (manifest + MDX + YAML); 46 с en
├── interview/      # ~1449 вопросов ru+en + exercises
├── deploy/         # шаблоны Caddyfile + systemd-юнит (bare-metal деплой)
├── scripts/        # deploy.sh (Node 22), sync/checks, контент-тулинг
...
- Build-time SEO-слой: `apps/web/scripts/{prerender,seo-artifacts,og}.mjs` вызываются из web `pnpm build` и пишут в `dist/` пререндер ~3000 HTML (темы ru+en, home, глоссарий DefinedTermSet, хабы flashcards/tracks/map/sandbox с crawlable body + CollectionPage, interview hub + ~1449×2 QAPage; без гидрации), sitemap.xml (lastmod + hreflang), robots.txt (private disallow + AI Allow), llms.txt / llms-full.txt / en/llms-full.txt, markdown-зеркала и OG-PNG (ru + `.en.png`). Absolute URLs: `VITE_SITE_URL` → fallback `VITE_API_BASE` (`deploy.sh` ставит оба на `https://$DOMAIN`). Пререндер и артефакты не должны попадать в SW-прекэш (`workbox.globIgnores` в `vite.config.ts`) и в eager-бандл. Данные для Node-скриптов лежат в json рядом с ts (`catalog-categories.data.json`, `glossary.data.json`, `tracks.data.json`): меняешь данные - меняй json, ts их импортирует.
...
| `PRESENCE_ANALYTICS`                                                                 | расширенная анонимная аналитика presence (uniques за всё время через HyperLogLog, 7д/30д, per-topic, `/api/presence/analytics` + страница `/analytics`); deploy-only, off по умолчанию (`deploy.sh` ставит `1`) |
...
- `scripts/deploy.sh` сам ставит Node 22/pnpm/Caddy, выставляет `VITE_SITE_URL`/`VITE_API_BASE`, генерит admin-секреты (`apps/api/scripts/gen-admin-secrets.mjs`), чистит build-зависимости и pnpm-store ради диска.
...
- **README-sync:** при глобальных изменениях функционала (новые/удалённые команды, модули, пакеты, зависимости, смена архитектуры или runtime, переименование точек входа) обнови `README.md` через `generate-readme` (с пересчётом LoC) и правила через `sync-project-rules`. Мелкие правки (опечатки, внутренний рефактор, багфиксы) README не трогают.
