...
| Источник | `code-counter` → `TOTAL` |
| Сокращение | при `N >= 1000` сокращать (правило ниже) |
| Маркеры | `<!-- loc:start -->` … `<!-- loc:end -->` (обязательны, не удалять) |
| Формат | `<img src=".../lines_of_code-{N}-lightgrey?style=flat">`, `{N}` - сокращённое значение |
...
**Сокращение (обязательно при `N >= 1000`):** floor до тысяч с суффиксом `k+` - `211332` → `211k+`, `5990` → `5k+`, `1024` → `1k+`. Миллионы - `M+` (`2 100 000` → `2M+`). Меньше 1000 - как есть. В URL `+` кодируется как `%2B`: `lines_of_code-211k%2B`; в `alt` читаемо - `211k+ lines of code`.
...
- [ ] LoC - 4-й бейдж в `<p>`-группе header, в маркерах, обновлён через `code-counter` → один ряд на GitHub; при `N >= 1000` сокращён до `k+` (`%2B` в URL).
