...
- **Открытость для поисковиков и ИИ-агентов.** Сборка пререндерит 84 статические страницы (46 тем ru, 29 en, главные и хабы) с полной теорией, JSON-LD (Course, BreadcrumbList, WebSite), canonical и hreflang; генерирует sitemap.xml, robots.txt, llms.txt, markdown-зеркала тем (`/topics/<slug>.md`) и OG-карточку на каждую тему в стиле обложки. Краулеры без JS видят полный контент, SPA поверх статики оживает без гидрации.
- **Два языка.** Русский (основной) и английский: 29 тем переведены, en-версии живут на отдельных адресах `/en/topics/<slug>` с hreflang-парами; переключение на лету.
