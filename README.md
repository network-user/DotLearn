...
- **46 тем, 269 концептов, каталог по разделам.** От полки «Основы: начни отсюда» (Python и JavaScript с нуля, регулярные выражения) и фронтенда (HTML/CSS) до продвинутого Python (ООП, дескрипторы, GIL, конкурентность), алгоритмы и структуры данных, базы данных (SQL, индексы, масштабирование), Web и API (включая HTTP с нуля), архитектура, ML и AI, инструменты (git, командная строка Linux). Плюс курируемые треки, фасетные фильтры и полнотекстовый поиск.
...
- **Интервальное повторение.** Флэшкарды с FSRS-планировщиком (ts-fsrs) по всем 46 темам.
...
http://localhost:5173 - в репозитории 46 тем (`python-basics`, `sql-fundamentals`, `javascript-basics`, `db-indexes` и др.).
